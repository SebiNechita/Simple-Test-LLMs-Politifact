"""
LLM-based Verdict Classification with Conformal Prediction using APS
...
MAX_SAMPLES = 22  # Set to None to use all data
...
def classify_with_conformal_aps(data, generator, threshold):
    """
    Classify statements with conformal prediction sets using APS.
    """
    predictions = []
    prediction_sets = []
    true_labels = []
    ...
    print(f"\nClassifying {len(data)} statements with APS conformal prediction...")
...
def evaluate_conformal_results(predictions, prediction_sets, true_labels, all_probs, data, results_folder=RESULTS_FOLDER, trial_num=None):
    ...
    print("\n" + "="*80)
    print(f"APS CONFORMAL PREDICTION RESULTS{trial_str}")
    print("="*80)
...
def main():
    ...
    if all_trial_metrics:
        print("\n" + "="*80)
        print("AGGREGATED APS RESULTS ACROSS ALL TRIALS")
        print("="*80)
